#!/bin/bash
# LFI Complete v1.0.0 — 单文件版本
# Linux Font Installer — 一行命令搞定Linux字体问题
# 用法: bash <(curl -sL https://github.com/sinyche/linux-font-installer/releases/download/v1.0.0/lfi-complete.sh)
...
detect_system() {
    ...
    ARCH=$(uname -m)
    if [ "$(id -u)" -eq 0 ]; then HAS_ROOT=true; else HAS_ROOT=false; fi
...
detect_windows() {
    ...
    for d in /mnt/Windows /mnt/windows; do
        [ -f "$d/System32/config/SOFTWARE" ] && WIN_VERSION="windows-unknown"
    done
    if [ ${#WIN_FONT_DIRS[@]} -gt 0 ]; then WIN_AVAILABLE=true; else WIN_AVAILABLE=false; fi
}
